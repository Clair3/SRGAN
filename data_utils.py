...
def train_lr_transform_with_lr(crop_size, upscale_factor):
    return Compose([
        Resize(crop_size // upscale_factor, interpolation=Image.BICUBIC),
        ToTensor()
    ])
...
class TrainDatasetFromFolderWithLR(Dataset):
    def __init__(self, datasetHR_dir, datasetLR_dir, crop_size, upscale_factor):
        ...
        self.imageHR_filenames = [join(datasetHR_dir, x) for x in listdir(datasetHR_dir) if is_image_file(x)]
    ...
    def __getitem__(self, index):
        hr_image = self.hr_transform(Image.open(self.imageHR_filenames[index]))
        lr_image = self.lr_transform(Image.open(self.imageLR_filenames[index]))
        return lr_image, hr_image
